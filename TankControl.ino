 ...
 * --Don't care-> Send on if on or off if off: Conclusion- Currently sending only OFF msg, ON not required--
...
void loop()
{
    ...
    if(!sensor_malfunction) {     //If sensor malfunction, do nothing
        ...
        if(!s2 && (!s3 || !s1)) {
            ...
            check_and_publish(TOPIC_MotorStatusChange, ON_SOLAR_TIMER, 0);
            onTimerFlag = 1;
            // if(!motor_state) {
            motor_state = 1;
