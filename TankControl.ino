...
bool s1, s2, s3, s1prev, s2prev, s3prev;  //Sensor values
...
void callback(char *msgTopic, byte *msgPayload, unsigned int msgLength) {
    ...
    if(!strcmp(msgTopic, TOPIC_SensorMalfunction)) {
        if(!strcmp(message, ON)) {
            ...
            EEPROM_Write_Flag = !EEPROM_write_and_commit(0, EEPROM_Value_To_Write);
...
void setup() {
    ...
    s1prev = s1 = !digitalRead(SENSOR1);  // Initial values needed for sensor state machine
    s2prev = s2 = !digitalRead(SENSOR2);
    s3prev = s3 = !digitalRead(SENSOR3);
...
void loop() {
    ...
    if(((!s3prev && !s2prev && !s1prev) && (!s3 && s2 && !s1)) || ((!s3prev && !s2prev) || (!s3 && s2))) {
